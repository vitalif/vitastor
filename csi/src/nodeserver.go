...
func (ns *NodeServer) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error)
{
    ...
    if (ns.method == MOUNT_UBLK && requiredBytes > 0)
    {
        err = resizeUblk(devicePath, requiredBytes)
        if (err != nil)
        {
            return nil, err
        }
    ...
    {
        // Ask the kernel to re-read the device size/partition table. This is best-effort and may
        // fail with EBUSY for mounted filesystems, but non-ublk devices may still update capacity.
        _, err = systemCombined("blockdev", "--rereadpt", devicePath)
        if (err != nil)
        {
            klog.Warningf("failed to re-read partition table for %s after resize: %v", devicePath, err)
        }
    }
