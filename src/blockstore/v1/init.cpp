...
int blockstore_init_journal::handle_journal_part(void *buf, uint64_t done_pos, uint64_t len)
{
    ...
    resume:
        while (pos < bs->journal.block_size)
        {
            ...
            if ((bs->journal.block_size-pos) < 12 || je->size < 12 /*crc,magic,type,size*/ ||
                je->magic != JOURNAL_MAGIC || buf_pos+je->size > len || je_crc32(je) != je->crc32 ||
            ...
            if (je->type == JE_SMALL_WRITE || je->type == JE_SMALL_WRITE_INSTANT)
            {
                    ...
                    clean_it->second.version < je->small_write.version)
                {
                    ...
                    bs->dirty_db.emplace(ov, (dirty_entry){
                        .state = (uint32_t)(BS_ST_SMALL_WRITE | BS_ST_SYNCED | (je->type == JE_SMALL_WRITE_INSTANT ? BS_ST_INSTANT : 0)),
                        .flags = 0,
                        .location = location,
            ...
            else if (je->type == JE_BIG_WRITE || je->type == JE_BIG_WRITE_INSTANT)
            {
                    ...
                    clean_it->second.version < je->big_write.version)
                {
                    ...
                    auto dirty_it = bs->dirty_db.emplace(ov, (dirty_entry){
                        .state = (uint32_t)(BS_ST_BIG_WRITE | BS_ST_SYNCED | (je->type == JE_BIG_WRITE_INSTANT ? BS_ST_INSTANT : 0)),
                        .flags = 0,
