...
vitastor-ublk поддерживает TRIM (discard): команды `blkdiscard` или `fstrim` на
подключённом устройстве удаляют объекты Vitastor, полностью покрытые очищаемым
диапазоном, и освобождают их место в пуле. Запись нулей (`blkdiscard -z`) тоже
поддерживается и так же освобождает место, если у образа нет родительских слоёв,
при этом гарантируя, что чтение вернёт нули. Подробнее о семантике см.
[примечания о TRIM](qemu.ru.md#trimdiscard).
