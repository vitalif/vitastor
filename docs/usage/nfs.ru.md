...
Однако, вы можете увеличить эту настройку целыми 3 способами:

1. Встроенной опцией `vitastor-nfs mount --readahead <SIZE>` (например, `--readahead 2M`).
2. Добавив в секцию `[nfsrahead]` файла конфигурации `/etc/nfs.conf` строку `nfs=2048` (глобальная
   настройка для всех NFS 3.0 точек монтирования).
3. Вручную следующей командой: \
   `echo 2048 > /sys/class/bdi/$(findmnt -no MAJ:MIN /mnt/nfs 2>/dev/null | tr -d ' ')/read_ahead_kb`

Здесь `2048` - размер readahead, а `/mnt/nfs` - путь к вашей ФС.

После увеличения параметра скорость линейного чтения легко может увеличиться в 2-3 раза, так что
