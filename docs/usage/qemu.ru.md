...
QEMU-драйвер Vitastor поддерживает TRIM (discard). Чтобы включить его, добавьте
`"discard":"unmap"` в опции blockdev (как в примерах выше) и `discard=unmap` в опции
дискового устройства гостя. После этого `fstrim` внутри виртуальной машины будет
освобождать место в пуле Vitastor и уменьшать занятое место (Used) образа.

TRIM в Vitastor - рекомендательная операция с гранулярностью в объект: удаляются
только объекты (блоки размером [block_size](../config/layout-cluster.ru.md#block_size),
умноженным на число чанков данных для EC-пулов, по умолчанию 128 КБ), полностью
покрытые очищаемым диапазоном, а более мелкие или невыровненные фрагменты диапазона
игнорируются. Также учтите, что если образ - клон, то в очищенной области снова могут
стать видны старые данные из родительских снимков вместо нулей. Это допустимо
семантикой TRIM, так как содержимое очищенных блоков не определено до следующей записи.

Драйвер также поддерживает эффективную запись нулей (WRITE_ZEROES), используемую,
например, опцией диска `detect-zeroes=unmap` и гостевыми системами, явно обнуляющими
блоки. Запись нулей требует выравнивания только на
[bitmap_granularity](../config/layout-cluster.ru.md#bitmap_granularity) (по умолчанию
4 КБ). Если у образа нет родительских снимков (слоёв), то объекты, полностью покрытые
запросом записи нулей, удаляются, что освобождает их место, а остаток диапазона
обнуляется обычными записями. В отличие от TRIM, чтение из обнулённой области после
завершения запроса гарантированно возвращает нули, поэтому запись нулей безопасна и
для клонов - данные родителя там маскируются физически записанными нулями.
