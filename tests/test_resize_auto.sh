...
console.log(JSON.stringify({
    ...
    entries: [ ...new Array(100).keys() ].map(i => ({
        ...
        writes: [ {
            ...
            type: 'big',
            stable: true,
            location: 128*1024*(($BLOCK_COUNT-100)+i),
            bitmap: "ffffffff",
            ext_bitmap: "ffffffff",
        } ],
    })),
...
if [[ -n "$OLD" ]]; then
    ...
    jq -S '. + {"entries": ([ .entries[] | (. + { "writes": [ .writes[0] + { "location": (.writes[0].location-1360*128*1024) } ] }) ] | sort_by(.stripe))}' < ./testdata/meta.json > ./testdata/1.json
fi
diff ./testdata/1.json ./testdata/2.json
if [[ -n "$OLD" ]]; then
    sudo build/src/disk_tool/vitastor-disk dump-journal --json --format data ${LOOP1}p1 | jq -S '[ .[] | del(.crc32, .crc32_prev) ]' > ./testdata/j2.json
...
if [[ -n "$OLD" ]]; then
    ...
    jq -S '. + {"entries": ([ .entries[] | (. + { "writes": [ .writes[0] + { "location": (.writes[0].location/128/1024
        | if . > '$BLOCK_COUNT'-100 then .-('$BLOCK_COUNT'-100+1) else '$BLOCK_COUNT'-100 end | .*128*1024) } ] }) ]) | sort_by(.stripe)}' < ./testdata/meta.json > ./testdata/1.json
fi
diff ./testdata/1.json ./testdata/2.json
if [[ -n "$OLD" ]]; then
    jq -S '[ .[] + {"valid":true} ]' < ./testdata/journal.json > ./testdata/j1.json
